...
import { soireeEvents, type SoireeEvent } from "@/lib/content";

function SoireeCard({ soiree }: { soiree: SoireeEvent }) {
  return (
    <Link
      href={`/expertises/soirees-evenementielles/${soiree.slug}`}
      className="group relative aspect-square overflow-hidden block"
      aria-label={`Découvrir la soirée ${soiree.name}`}
    >
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          <p className="font-inter text-white/60 text-lg max-w-2xl mx-auto">
            {soireeEvents.length} formules de soirées événementielles sur mesure. Cliquez sur une carte pour découvrir l'animation en détail.
          </p>
        </div>
        ...
        <div className="flex items-center gap-4 mb-10">
          ...
          <span className="font-inter text-sm text-white/40">
            {soireeEvents.length} formules
          </span>
          <div className="h-px flex-1" style={{ background: "rgba(201,169,97,0.3)" }} />
        </div>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 lg:gap-4">
          {soireeEvents.map((soiree) => (
            <SoireeCard key={soiree.slug} soiree={soiree} />
          ))}
        </div>
      </div>
    </section>
